...
(defn insert-nest!
  ...
  (let [entry {:street (:street entry-map)
               :house_number_name (:house_number_name entry-map)
               :gps (:gps entry-map)
...
(def sqlmap {:select [:id :street :house_number_name]
             :from [:nests]
             :where [:> :id 1]})

(def test-map (-> (insert-into :nests)
                  (columns :street :house_number_name :gps :species :height :facing :type :date :destroyed :destroyed-date)
                  (values
